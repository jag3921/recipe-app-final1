...
app.get('/api/recipes/:word', (req, res) => {
  ...
  const request = unirest.get(`https://api.spoonacular.com/recipes/complexSearch?query=${word}&maxFat=25&number=25&apiKey=${API_KEY}`) //eslint-disable-line
    .then((response) => {
...
app.get('/api/review/:id', (req, res) => {
  ...
  const request = unirest.get(`https://api.spoonacular.com/recipes/${id}/information?apiKey=${API_KEY}`) //eslint-disable-line
    .then((response) => {
      const results = response.body;
      res.json(results);
    })
...
app.get('/getReviews', (req, res) => { //eslint-disable-line
